...
describe('init config validation', () => {
  it('accepts a minimal valid config', () => {
    client = new AbtoBrowserClient({ projectKey: 'pk_test' });
    expect(client.config.projectKey).toBe('pk_test');
  ...
  it('rejects a malformed apiHost', () => {
    expect(() => new AbtoBrowserClient({ projectKey: 'pk_test', apiHost: 'not a url' })).toThrowError(
      '[abto] apiHost is not a valid http(s) URL: "not a url"',
    );
  });

  it('rejects a malformed endpoint', () => {
    expect(() => new AbtoBrowserClient({ projectKey: 'pk_test', endpoint: 'nope' })).toThrowError(
  ...
  it('uses privacy-preserving capture defaults', () => {
    ...
    expect(client.config.hashSalt).toBe('pk_test');
  });
});
