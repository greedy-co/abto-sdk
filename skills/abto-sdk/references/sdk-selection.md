...
| Runtime | Responsibility | Package | Credential |
|---|---|---|---|
| Browser JavaScript | Autocapture, custom events, identity, client outcomes | `@abto-app/event` | Event Key |
| Node.js backend | Gateway calls and request context | `@abto-app/calling` | Calling Key plus provider keys |
| Python backend | Gateway calls and request context | `abto[openai]` | Calling Key plus provider keys |
| Flutter/Dart native app | App events and client outcomes | `abto` | Event Key |
| Android/Kotlin app | App events and client outcomes | `app.abto:abto-app` | Event Key |
| iOS/macOS app | App events and client outcomes | `AbtoApp` | Event Key |
